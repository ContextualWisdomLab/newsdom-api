    ...
    async def __call__(self, scope, receive, send):
        ...
        try:
            ...
            if str(e) == "Payload Too Large":
                # Only possible if the app hasn't sent the response yet, but Starlette might have sent headers.
                # If Starlette's parser caught it... Wait, if the parser is reading it, the app didn't send yet
                try:
                    await self._send_413(send)
                except Exception:
                    pass
...
resp = httpx.post("http://127.0.0.1:8016/", content=generate_chunked(), headers={"Content-Type": "multipart/form-data; boundary=boundary"})
