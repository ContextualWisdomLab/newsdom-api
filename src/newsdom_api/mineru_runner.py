...
# Language-agnostic defaults. ``auto`` lets MinerU detect the document language
# and lets born-digital (text-layer) PDFs skip forced OCR.
...
# Conservative allow-list for MinerU language codes (e.g. ``auto``, ``en``,
# ``japan``, ``korean``, ``ch_server``). Anything outside this shape is rejected
# to keep untrusted input away from the subprocess argv.
...
def normalize_language(language: str) -> str:
    ...
    Returns the lower-cased language code when it matches the supported shape
    and raises :class:`ValueError` otherwise so callers can surface a client
    error instead of forwarding untrusted input to the subprocess.
    """
...
def _find_output_dir(base_output_dir: Path, method: str = DEFAULT_MODE) -> Path:
    ...
    for candidate_method in search_order:
        candidates = list(base_output_dir.glob(f"*/{candidate_method}"))
        if candidates:
            return candidates[0]
