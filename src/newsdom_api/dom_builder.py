...
def _build_pages_without_page_idx(
    ...
    if len(page_info_by_idx) > 1:
        ...
        pages = []
        # ⚡ Bolt: Iterate over sorted items instead of sorting keys and performing dictionary lookups inside the loop.
        # This safely preserves sequential sorting while avoiding redundant hash map lookups.
...
def _build_pages_with_page_idx(
    ...
    pages = []
    article_seq = count(1)
    # ⚡ Bolt: Iterate over sorted items to prevent redundant hash map lookups on every iteration.
    # Tuple comparison in sorting short-circuits on the key, preserving identical sequential behavior.
