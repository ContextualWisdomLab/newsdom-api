...
def _build_pages_without_page_idx(
    ...
    if len(page_info_by_idx) > 1:
        ...
        pages = []
        # ⚡ Bolt: Use sorted(dict.items()) to avoid redundant O(1) hash map lookups inside the hot loop.
        # Since dict keys are strictly unique, tuple comparison short-circuits and preserves the exact sorting order.
