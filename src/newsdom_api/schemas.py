...
class ImageNode(BaseModel):
    ...
    captions: List[CaptionNode] = Field(
        ...
        json_schema_extra={"example": [{"text": "写真：昨日の大雨の様子"}]},
    )
...
class ArticleNode(BaseModel):
    ...
    images: List[ImageNode] = Field(
        ...
        json_schema_extra={
            "example": [{"path": "images/article_123_fig1.jpg", "media_type": "image"}]
        },
    )
    captions: List[CaptionNode] = Field(
        default_factory=list,
...
class PageNode(BaseModel):
    ...
    articles: List[ArticleNode] = Field(
        ...
        json_schema_extra={
            "example": [
                {
                    "article_id": "section-20231015-001",
                    "headline": "Quarterly results",
                    "body_blocks": ["First paragraph."],
                }
            ]
        },
    )
    ...
    headers: List[str] = Field(
        ...
        json_schema_extra={"example": ["The Daily News"]},
    )
...
class ParseResponse(BaseModel):
    ...
    quality: ParseQuality = Field(
        ...
        json_schema_extra={
            "example": {"status": "success", "parser": "mineru", "warnings": []}
        },
    )
