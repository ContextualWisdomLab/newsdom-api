...
def _parse_access_failure(request: Request) -> JSONResponse | None:
    ...
    expected_token_bytes = token.encode("utf-8")
    if len(credentials) != len(expected_token_bytes):
        hmac.compare_digest(expected_token_bytes, expected_token_bytes)
        return _unauthorized_response()

    if not hmac.compare_digest(credentials, expected_token_bytes):
        return _unauthorized_response()
    return None
