...
app = FastAPI(
    title="NewsDOM API",
    description="DOM-style parser API for scanned Japanese newspaper PDFs using MinerU OCR.",
    version="0.2.0",
)
...
@app.post("/parse", response_model=ParseResponse, tags=["Document Processing"], summary="Parse PDF Document")
async def parse(
    file: Annotated[
        UploadFile,
        File(description="A scanned Japanese newspaper PDF document to be parsed.")
    ]
) -> ParseResponse:
