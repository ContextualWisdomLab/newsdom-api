...
def validate_dom(json_path: Path) -> bool:
    ...
    try:
        ...
        print(f"JSON Decode Error in {json_path.name}:", file=sys.stderr)
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        is_valid = validate_dom(args.input)
        if is_valid:
            print(f"Successfully validated {args.input.name}")
        else:
            sys.exit(1)
