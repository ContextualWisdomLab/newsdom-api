...
def export_jsonl(json_path: Path, output_path: Path) -> None:
    ...
    # Ensure atomic publish by writing to a temporary file first
    temp_dir = output_path.parent if output_path.parent.exists() else None
    temp_fd, temp_path_str = tempfile.mkstemp(dir=temp_dir, text=True)
