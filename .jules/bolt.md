...
## 2024-07-31 - Fast asynchronous file uploads
**Learning:** For asynchronous file uploads in FastAPI/Starlette, using small chunk sizes (e.g., 8192 bytes) with `await file.read()` creates massive threadpool and context-switching overhead, significantly slowing down upload processing.
**Action:** Default to larger chunk sizes (e.g., 1MB / `1024 * 1024`) when reading uploaded files asynchronously to maximize throughput and reduce overhead.
