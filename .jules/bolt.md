...
## 2024-05-24 - 불필요한 딕셔너리 중복 조회 제거
**Learning:** 딕셔너리에서 동일한 키를 반복해서 조회하는 것은(예: `payload.get("key")`를 타입 검사와 할당에 각각 사용) 성능에 오버헤드를 줍니다.
**Action:** 동일한 키를 여러 번 조회해야 할 경우, 지역 변수에 값을 캐싱하여 중복된 딕셔너리 조회를 제거하고 실행 속도를 개선합니다.
## 2024-08-30 - 딕셔너리 반복문에서 `.items()`를 통한 중복 조회 제거
**Learning:** 딕셔너리의 키를 정렬하여 반복(`for key in sorted(dict):`)한 뒤 루프 내부에서 `dict[key]` 또는 `dict.get(key)`로 값을 다시 조회하면 불필요한 해시맵 룩업 오버헤드가 발생합니다.
**Action:** `for key, value in sorted(dict.items()):` 패턴을 사용하여 키와 값을 동시에 추출함으로써 반복문 내 중복된 딕셔너리 조회를 제거하고 실행 속도를 개선합니다.
