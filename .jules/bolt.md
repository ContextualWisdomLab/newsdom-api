...
## 2024-08-08 - Increase upload chunk size for asynchronous file reads
**Learning:** For asynchronous file uploads in FastAPI/Starlette, avoid small chunk sizes (e.g., 8192 bytes) with `await file.read()`, as it creates massive threadpool and context-switching overhead for large uploads like PDFs.
**Action:** Improve performance by using larger chunk sizes (e.g., 1MB) defined as a named constant (e.g., `UPLOAD_READ_CHUNK_SIZE_BYTES = 1024 * 1024`).
