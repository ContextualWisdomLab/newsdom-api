...
## 2026-09-17 - [Avoid memory allocation in string truthiness checks]
**Learning:** Using `bool(s.strip())` allocates a new string object in memory, which increases garbage collection pressure in high-frequency text evaluation paths.
**Action:** Use `not s.isspace()` instead of `bool(s.strip())` to check if a string contains non-whitespace characters without allocating a new string.
