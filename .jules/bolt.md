...
## 2026-08-12 - Optimize file read chunk size for async file upload
**Learning:** For asynchronous file uploads in FastAPI/Starlette, avoid small chunk sizes (e.g., 8192 bytes) with `await file.read()`, as it creates massive threadpool and context-switching overhead. Improve performance by using larger chunk sizes (e.g., 1MB).
**Action:** Use a named constant (e.g., `UPLOAD_READ_CHUNK_SIZE_BYTES = 1024 * 1024`) for chunk sizes to prevent test failures and avoid small chunks when reading async files.
