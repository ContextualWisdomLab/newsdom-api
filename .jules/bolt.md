...
## 2024-10-27 - Early truthiness and isspace() over strip() for space checks
**Learning:** Checking `bool(text.strip())` on a string allocates a new string just to evaluate truthiness. In Python, `isspace()` checks if all characters are whitespace in-place.
**Action:** When determining if a string contains visible characters, use an early truthiness check combined with `not text.isspace()` (e.g., `bool(text) and not text.isspace()`) to eliminate unnecessary string allocation in hot paths.
