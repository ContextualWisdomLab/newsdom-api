...
## 2024-05-24 - Fast JSON Parsing
**Learning:** When reading JSON files from disk, passing `path.read_bytes()` directly to `json.loads()` is faster than `path.read_text(encoding='utf-8')` because it bypasses the intermediate string allocation and decoding overhead.
**Action:** Use `.read_bytes()` instead of `.read_text()` when loading JSON from files in performance-critical paths.
