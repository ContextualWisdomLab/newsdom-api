...
## 2024-05-24 - Optimal Chunk Size for Uploads
**Learning:** In FastAPI, reading large file uploads with a small chunk size (e.g., 8KB) via `await file.read(8192)` causes excessive asynchronous event loop iterations and thread-dispatch overhead. Increasing the chunk size (e.g., to 1MB: `await file.read(1024 * 1024)`) significantly reduces overhead and improves I/O performance.
**Action:** Use a larger chunk size like 1MB when processing file uploads asynchronously to avoid excessive event loop blocking and thread dispatching.
