...
## 2026-07-30 - Large chunk sizes for async file uploads
**Learning:** For asynchronous file uploads in FastAPI/Starlette, using small chunk sizes (like 8192 bytes) with `await file.read()` creates massive threadpool and context-switching overhead.
**Action:** Default to larger chunk sizes (e.g., 1MB / `1024 * 1024`) to significantly improve throughput and reduce overhead.
