...
## 2026-07-25 - Direct bytes read for JSON load
**Learning:** Passing `path.read_bytes()` directly to `json.loads()` is faster than `path.read_text(encoding='utf-8')` because it bypasses the intermediate string allocation and decoding overhead, delegating it down to the C implementation.
**Action:** When reading JSON files from disk, use `path.read_bytes()` for performance.
