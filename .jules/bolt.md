...
## 2026-06-26 - Unroll fixed-size array validation in hot paths
**Learning:** Using generator expressions, tuple allocations, and `any()` checks when extracting coordinates or processing fixed-size arrays inside hot parsing loops introduces unnecessary allocation and iteration overhead. Profiling indicates that replacing these structures with sequential unrolled variables enables immediate early returns upon invalid data (e.g. failing fast on the first coordinate).
**Action:** When extracting and validating known, fixed-size data structures like 4-element bounding boxes, unroll the extraction and validation logic rather than using high-level Python iteration primitives.
