...
## 2024-07-31 - Avoid allocation with string.isspace()
**Learning:** Using `.strip()` followed by a truthiness check (`bool(headline.strip())`) to verify if a string contains visible text requires a new string allocation for the stripped result. In hot parsing loops, checking truthiness and then using `not headline.isspace()` avoids this memory allocation and provides a measurable speedup.
**Action:** To avoid unnecessary string allocations when checking if a string contains visible text, use early truthiness and `.isspace()` checks instead of `.strip()`.
