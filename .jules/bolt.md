...
## 2024-05-24 - Avoid redundant decode allocation when loading JSON
**Learning:** Passing `path.read_text(encoding="utf-8")` to `json.loads` incurs unnecessary string allocation and decoding overhead in Python.
**Action:** When loading JSON from files, use `json.loads(path.read_bytes())` instead to bypass intermediate string allocations and improve parsing speed.
