...
## 2026-08-16 - Avoid redundant dictionary lookups
**Learning:** Calling `payload.get("key")` multiple times in a single statement (e.g. for type checking and assignment) causes redundant hash map lookups and function calls.
**Action:** Cache the result of dictionary lookups in a local variable to reduce overhead.
