...
## 2024-05-25 - Pre-compile Regex at the Module Level
**Learning:** Instantiating `re.compile()` or calling `re.sub()` directly inside high-frequency functions or loops adds unnecessary overhead by repeatedly parsing the regex string on every execution.
**Action:** Extract regular expressions and pre-compile them at the module level using `re.compile()` to parse them only once.
