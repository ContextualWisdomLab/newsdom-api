...
## 2026-09-18 - [Optimize string truthiness checks]
**Learning:** Using `not s.isspace()` instead of `bool(s.strip())` for string truthiness checks avoids allocating a new string object in memory, reducing garbage collection pressure in high-frequency text evaluation paths.
**Action:** Always prefer `isspace()` over `strip()` when checking if a string contains non-whitespace characters.
