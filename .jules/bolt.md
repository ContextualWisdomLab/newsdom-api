...
## 2026-09-15 - Use `not .isspace()` for string truthiness checks
**Learning:** Using `bool(s.strip())` to check if a string contains non-whitespace characters allocates a new string object in memory, adding unnecessary garbage collection pressure in high-frequency text evaluation paths.
**Action:** In Python, use `not s.isspace()` instead of `bool(s.strip())` for string truthiness checks to avoid allocating a new string object in memory.
