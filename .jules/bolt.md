...
## 2024-07-25 - Avoid small chunk sizes for async file uploads
**Learning:** For asynchronous file uploads in FastAPI/Starlette, small chunk sizes (e.g., 8192 bytes) with `await file.read()` create massive threadpool and context-switching overhead.
**Action:** Default to larger chunk sizes (e.g., 1MB / `1024 * 1024`) for significantly better performance in file upload loops.
