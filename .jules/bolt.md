...
## 2026-08-29 - FastAPI Async Upload Chunk Size
**Learning:** For asynchronous file uploads in FastAPI/Starlette, using small chunk sizes (e.g., 8192 bytes) with `await file.read()` creates massive threadpool and context-switching overhead, severely degrading performance for large payloads.
**Action:** Use larger chunk sizes (e.g., 1MB) defined as a named constant to reduce event loop iterations and context switches during file streaming.
