...
## 2024-08-28 - 중복 딕셔너리 조회를 지역 변수에 캐싱하여 최적화
**학습:** 딕셔너리에서 같은 키를 조회할 때 (예: 타입 확인을 위해 한 번, 변수에 할당하기 위해 한 번) 중복해서 `payload.get('key')`를 호출하면 불필요한 오버헤드가 발생합니다.
**실행:** 딕셔너리 조회 결과를 지역 변수에 할당(캐싱)한 뒤 이를 재사용하여 불필요한 중복 조회를 방지합니다.
