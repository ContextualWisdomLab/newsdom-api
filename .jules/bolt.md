...
## 2026-07-31 - Replace Path.read_text() with Path.read_bytes() for JSON loading
**Learning:** Python's `json.loads` accepts both strings and bytes. Using `path.read_text(encoding="utf-8")` creates an intermediate string in memory that `json.loads` must then process. Passing the bytes directly via `path.read_bytes()` avoids this intermediate string allocation and decoding overhead, which offers a small but noticeable performance improvement (and avoids potential UnicodeDecodeErrors during the text read phase if the JSON happens to have invalid surrogate pairs that `json.loads` would handle differently or gracefully).
**Action:** When reading JSON files from disk, use `json.loads(path.read_bytes())` instead of `json.loads(path.read_text(encoding="..."))` for improved performance. Remember to update any mock patches (e.g. in tests) from `read_text` to `read_bytes` to prevent test breakages.
