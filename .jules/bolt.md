...
## 2024-08-01 - Avoid allocating strings during text visibility checks
**Learning:** Calling `.strip()` on strings just to check if they contain visible text allocates new string objects unnecessarily.
**Action:** Use `not text.isspace()` combined with a truthiness check (e.g. `bool(text) and not text.isspace()`) instead of `bool(text.strip())` to prevent string allocations in hot paths.
