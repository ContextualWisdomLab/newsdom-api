## 2026-08-12 - Enhance Developer UX with OpenAPI Schema Examples
...
## 2026-08-12 - Pydantic V2에서의 명시적 `...` 생략 및 `json_schema_extra` 사용

**Learning:** 백엔드 전용 프로젝트에서는 OpenAPI/Swagger 문서의 질이 곧 DX(Developer Experience)를 결정합니다. FastAPI와 Pydantic V2를 사용할 때, 필수 필드라도 기본값이 지정되지 않으면 자동으로 필수(Required)로 처리되므로, `Field(...)`나 `File(...)`와 같이 명시적인 생략 기호(`...`)를 사용하는 것은 중복되며 코드를 지저분하게 만듭니다. 또한, 폼 파라미터(`Form`)나 필드에 `json_schema_extra={"example": "..."}`를 추가하면 생성된 OpenAPI 스키마에 구체적인 예제가 포함되어 개발자가 API를 이해하고 테스트하기 훨씬 수월해집니다.
**Action:** 앞으로 Pydantic V2 기반의 스키마와 FastAPI 라우터 파라미터를 정의할 때는 필수 필드에 불필요한 `...`을 생략하고, 직관적인 `json_schema_extra` 예제 데이터를 적극적으로 추가하여 DX를 향상시켜야 합니다.
