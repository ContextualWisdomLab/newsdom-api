...
## 2024-05-18 - [Swagger UI CSP Fix]
**Learning:** Strict CSP (`default-src 'none'`) breaks API documentation endpoints (Swagger UI/ReDoc) as they require inline scripts, styles, and external assets (like jsdelivr and google fonts) to render properly. This impacts developer UX heavily.
**Action:** Always conditionally relax the `Content-Security-Policy` header specifically for documentation routes (`/docs`, `/redoc`, `/openapi.json`) in FastAPI applications while keeping the strict policy for other endpoints to maintain security and functionality.
