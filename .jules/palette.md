...
## 2026-08-31 - Swagger UI Authentication Persistence

**Learning:** When developing a backend API that relies heavily on Bearer token authentication via Swagger UI, developer experience (DX) suffers if the token is lost on every page refresh. Enabling `persistAuthorization` in FastAPI's Swagger UI parameters trivially solves this by storing the credentials locally.
**Action:** Always include `"persistAuthorization": True` in FastAPI `swagger_ui_parameters` when authentication is enforced, to streamline local development and API testing.
