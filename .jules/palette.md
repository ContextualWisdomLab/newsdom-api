## 2026-09-09 - FastAPI Swagger UI Content Security Policy Relaxation
...
## 2026-09-09 - FastAPI Swagger UI Content Security Policy Relaxation
**Learning:** By default, FastAPI's Swagger UI functionality relies on loading external assets (scripts and styles from cdn.jsdelivr.net, fonts from googleapis, images from fastapi.tiangolo.com). When applying a strict API-wide Content-Security-Policy (`default-src 'none'`), the Swagger UI and ReDoc pages break. Furthermore, requiring developers to re-authenticate repeatedly degrades Developer Experience (DX).
**Action:** Relaxed the Content-Security-Policy conditionally for documentation endpoints (`/docs`, `/redoc`, `/openapi.json`, `/docs/oauth2-redirect`) while maintaining strict limits for core API routes. Added `persistAuthorization: True` conditionally for development environments to improve DX while preserving production security.
