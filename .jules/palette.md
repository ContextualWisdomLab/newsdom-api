## 2026-09-13 - Enhance Developer UX with OpenAPI Schema Examples
...
## 2026-09-13 - 개발 환경 프로필의 Swagger UI에 persistAuthorization 추가
**Learning:** `swagger_ui_parameters`에 `"persistAuthorization": True`를 추가하면 Swagger UI 페이지를 새로고침할 때마다 인증 자격 증명이 유지되어 개발자 경험(DX)이 향상됩니다.
**Action:** 인증이 필요한 FastAPI 애플리케이션 작업 시, 수동 테스트 중에 토큰을 반복해서 입력하는 것을 피하면서 프로덕션 보안 요구 사항을 준수하기 위해 이 설정이 조건부로(예: 개발 환경에서) 활성화되어 있는지 확인합니다.
