...
## 2026-08-23 - Swagger UI 인증 상태 유지 (persistAuthorization)
**Learning:** FastAPI의 Swagger UI(docs)에서 페이지를 새로고침할 때마다 Bearer 토큰이 초기화되어 개발자들이 반복적으로 토큰을 입력해야 하는 UX/DX 문제가 자주 발생함을 인지함.
**Action:** `swagger_ui_parameters`에 `"persistAuthorization": True` 옵션을 추가하여 브라우저 세션 동안 인증 상태를 유지함으로써 문서 테스트 편의성을 향상시킴.
