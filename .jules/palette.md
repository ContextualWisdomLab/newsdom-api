...
## 2026-09-07 - Improve Developer Experience with persistAuthorization

**Learning:** In FastAPI Swagger UI, enabling `"persistAuthorization": True` greatly improves Developer Experience (DX) by preventing authentication loss on page refreshes. However, to maintain production security, this convenience feature should only be enabled in development environments (e.g., `RuntimeProfile.DEVELOPMENT`).
**Action:** When configuring Swagger UI in backend projects with authentication, conditionally enable `persistAuthorization` based on the environment to balance developer convenience and production security.
