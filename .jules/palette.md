...
## 2026-08-19 - Swagger UI 인증 유지 추가

**Learning:** 백엔드 API에서 개발자 경험(DX)을 향상시키기 위해 Swagger UI의 인증 상태를 유지(`persistAuthorization: True`)하는 것이 유용함을 알게 되었습니다. 페이지 새로고침 시마다 토큰을 재입력해야 하는 번거로움을 줄여줍니다.
**Action:** FastAPI와 같은 프레임워크를 사용할 때 OpenAPI 문서의 `swagger_ui_parameters` 설정을 검토하고, 인증 유지를 활성화하여 개발자 친화적인 환경을 구성할 것입니다.
