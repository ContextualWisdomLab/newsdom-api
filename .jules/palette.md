...
## 2026-09-06 - Swagger UI Authentication Persistence (Developer UX)

**Learning:** In backend-only projects, DX is UX. Adding `persistAuthorization=True` to Swagger UI parameters drastically reduces developer friction during API testing by preserving the Bearer token across page reloads. However, this must be gated behind a development profile check to ensure production environments remain secure and tokens are not persisted unnecessarily.
**Action:** When configuring Swagger UI for FastAPI projects requiring authentication, conditionally enable `persistAuthorization` based on the environment to balance developer convenience and security.
