...
## 2026-09-08 - Swagger UI Authentication Persistence
**Learning:** In FastAPI applications, adding `"persistAuthorization": True` to `swagger_ui_parameters` improves Developer Experience (DX) by preserving authentication credentials across Swagger UI page refreshes.
**Action:** Ensure this is only enabled in development environments (e.g., `RuntimeProfile.DEVELOPMENT`) to adhere to production security requirements while improving developer usability.
