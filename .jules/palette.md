...
## 2026-07-28 - [FastAPI Route Parameter OpenAPI Examples]
**학습:** API 경로 의존성(예: `Form`)에 `json_schema_extra={"example": ...}`를 추가하면 Swagger UI에서 개발자가 직관적으로 페이로드 예시를 볼 수 있어 DX(Developer Experience)가 크게 향상됩니다.
**실행:** FastAPI의 라우트 파라미터(`Form`, `Query` 등) 정의 시 `description`과 함께 `json_schema_extra={"example": [...]}`를 항상 추가하여 풍부한 OpenAPI 문서를 제공합니다.
