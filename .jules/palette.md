...
## 2026-09-05 - Swagger UI persistAuthorization 개발자 경험 개선

**Learning:** API를 개발할 때, 개발 환경에서 Swagger UI를 사용할 때 페이지 새로고침마다 인증 토큰이 날아가는 것은 매우 번거로운 경험입니다. Swagger UI 설정에 `persistAuthorization: True`를 추가하면 이러한 불편함을 해소할 수 있으며, 이는 프론트엔드가 없는 백엔드 프로젝트에서 훌륭한 DX(Developer Experience) / UX 개선 사례입니다. 단, 보안을 위해 프로덕션 환경에서는 비활성화해야 합니다.
**Action:** 향후 FastAPI 프로젝트에서는 런타임 프로필(예: Development)에 따라 조건부로 Swagger UI의 `persistAuthorization` 옵션을 활성화하여 보안과 개발자 경험을 모두 만족시키도록 할 것입니다.
