...
**Vulnerability:** Passing strings containing non-ASCII characters directly to `hmac.compare_digest()` raises a `TypeError`, causing a 500 Internal Server Error. This can be exploited by an attacker sending malformed Authorization headers (e.g., `Bearer 漢字`) to cause a Denial of Service (DoS) on protected endpoints.
