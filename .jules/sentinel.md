...
## 2025-03-10 - Prevent 500 Errors from Non-ASCII Bearer Tokens
**Vulnerability:** Python's `hmac.compare_digest()` raises a `TypeError` if passed strings containing non-ASCII characters, which can lead to unhandled 500 Internal Server Errors when checking Bearer tokens on protected endpoints.
**Learning:** Security token comparison endpoints can fail open or crash (DoS) if client-supplied header strings contain non-ASCII payloads that hit strict string-comparison functions like `hmac.compare_digest()`.
