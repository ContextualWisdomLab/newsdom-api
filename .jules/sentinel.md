...
## 2026-08-31 - 🛡️ Prevent DoS via Memory Exhaustion in FastAPI Form Fields
**Vulnerability:** Textual `Form` fields (`language` and `mode` in `/parse`) lacked `max_length` limits. Since `python-multipart` loads the entire form data into memory before route execution, attackers could cause memory exhaustion (DoS) by sending massively padded form text fields.
**Learning:** In FastAPI, payload size limits evaluated inside the route body do not protect against massive `Form` fields, as the multipart parsing happens beforehand.
**Prevention:** Always set explicit `max_length` boundaries on textual `Form` fields (e.g., `Form(max_length=64)`).
