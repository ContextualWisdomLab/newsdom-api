...
## 2026-08-17 - DoS vulnerability via unbound FastAPI Form fields
**Vulnerability:** FastAPIs Form fields `language` and `mode` lacked `max_length` constraints, allowing potential memory exhaustion DoS since `python-multipart` loads the entire form data into memory.
**Learning:** Textual form fields must always have explicit length limits to prevent DoS, even when default values exist.
**Prevention:** Always use `Form(max_length=...)` for textual inputs.
