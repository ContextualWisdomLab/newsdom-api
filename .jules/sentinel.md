...
## 2026-08-01 - Trivy CI Vulnerability Fixes for Outdated Dependencies
**Vulnerability:** The repository's `uv.lock` file contained multiple outdated Python dependencies (e.g., `pillow`, `pypdf`, `setuptools`) triggering CRITICAL and HIGH severity findings during the GitHub Actions `trivy fs` scan. Additionally, an upstream vulnerability in `pymdown-extensions` (CVE-2026-61632) could not be resolved because the parent package `mkdocs-material` restricts the version to `<11.dev0`.
**Learning:** Directly upgrading dependencies via `uv lock --upgrade-package <package_name>` effectively resolves most standard Trivy findings. For transitive or upstream constraint conflicts that prevent upgrading (like `pymdown-extensions`), the vulnerability must be temporarily suppressed in the root `.trivyignore` file, along with a documented justification and revisit date, to allow the CI pipeline to pass.
**Prevention:** Regularly run security dependency scans locally and address them proactively before they block CI pipelines. Ensure `.trivyignore` format complies with repository tests (specifying rule ID, affected artifact, reason, and revisit date) when suppression is absolutely necessary.
