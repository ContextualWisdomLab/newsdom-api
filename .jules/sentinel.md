...
## 2026-09-09 - Specify URL Schemes in CSP to Prevent SAST Substring Mismatches
**Vulnerability:** A relaxed Content-Security-Policy (CSP) that permits external CDNs by domain alone (e.g., `cdn.jsdelivr.net`) can be flagged by SAST tools like CodeQL as an "Incomplete URL substring sanitization" vulnerability. This is because a bare domain might be interpreted broadly or embedded insecurely without an explicit protocol constraint.
**Learning:** When relaxing CSP rules to allow specific third-party assets (like for Swagger UI/ReDoc), always include the explicit `https://` scheme before the domain name (e.g., `https://cdn.jsdelivr.net`).
**Prevention:** Hardcode the `https://` prefix for all external `script-src`, `style-src`, `img-src`, and `font-src` directives in the CSP header.
