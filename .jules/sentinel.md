...
## 2026-09-26 - Bound validated parser selector fields
**Vulnerability:** Unbounded `language` and `mode` values could reach parser-selector normalization even though the supported values are short identifiers.
**Learning:** FastAPI field limits bound the validated application value but do not prevent the multipart parser from receiving the request body first. They must not be presented as a complete request-memory defense.
**Prevention:** Keep explicit form-field limits and focused 422 contracts, while retaining transport request-body and multipart part limits as separate controls.
