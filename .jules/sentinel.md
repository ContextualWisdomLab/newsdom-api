...
## 2026-09-19 - [CRITICAL] Update anyio and httpx2 to fix CVE-2026-63374 and CVE-2026-84381/CVE-2026-84382
**Vulnerability:** The `anyio` dependency version was vulnerable to CVE-2026-63374 (TLSStream IDNA 2003 host name encoding enables potential TLS certificate spoofing). `httpx2` and `httpcore2` were vulnerable to CVE-2026-84381 (WebSocket traffic sent in plaintext via SOCKS5 proxy) and CVE-2026-84382 (Denial of Service via streaming response decompression memory amplification).
**Learning:** Development dependencies used for testing and documentation can introduce critical vulnerabilities into the project environment and CI pipelines, even if they aren't deployed to production. Regular vulnerability scanning (e.g., using `trivy`) of the entire dependency tree, including `dev` and `docs` groups, is essential.
**Prevention:** Continuously monitor and update dependencies in `pyproject.toml` and lock files. Use automated tools to enforce version constraints that exclude known vulnerable versions (e.g., `anyio>=4.14.2`, `httpx2>=2.12.0`).
