...
## 2024-05-18 - Unhandled TypeError DoS in hmac.compare_digest
**Vulnerability:** Comparing an untrusted header containing non-ASCII characters against a string using `hmac.compare_digest` throws a `TypeError`, causing a 500 Internal Server Error, which can lead to a Denial of Service (DoS) vulnerability.
**Learning:** `hmac.compare_digest` does not support comparing strings with non-ASCII characters, and unhandled exceptions in FastAPI authentication dependencies fail the request before endpoint execution without proper error wrapping.
**Prevention:** Always encode strings to bytes (e.g., using `.encode('utf-8')`) before passing them to `hmac.compare_digest()`.
