...
**Learning:** Security-sensitive functions in the standard library may have implicit type constraints or behavior with non-ASCII strings. Relying on FastAPI/Starlette to implicitly sanitize headers is dangerous because HTTP headers can technically contain arbitrary byte sequences.
**Prevention:** When comparing cryptographic strings or secrets with `hmac.compare_digest`, explicitly encode both strings into bytes (e.g., `.encode("utf-8")`) before comparison to ensure constant-time checking and prevent unhandled type errors.
