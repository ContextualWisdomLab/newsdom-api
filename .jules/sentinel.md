...
## 2026-07-07 - [Path Traversal Prevention in File Uploads]
**Learning:** Passing user-controlled filenames directly into backend processing functions without sanitization is a critical path traversal vulnerability that security scanners like Strix will flag.
**Action:** Use `os.path.basename` to strip directory components from filenames when handling user uploads.
