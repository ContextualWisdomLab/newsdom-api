...
## 2026-07-30 - Fix DoS vulnerability in hmac.compare_digest
**Vulnerability:** HTTP Authorization header containing non-ASCII characters causes a 500 Internal Server Error (DoS) due to unhandled TypeError in `hmac.compare_digest`.
**Learning:** `hmac.compare_digest()` raises a `TypeError` if provided with strings containing non-ASCII characters. It does not safely handle UTF-8 inputs implicitly.
