...
## 2026-09-26 - [Docker Healthcheck Security Enhancement]
**Vulnerability:** The test and runtime Dockerfiles were missing an explicit `HEALTHCHECK` directive. While not a direct vulnerability, this is flagged by security scanners (e.g., Trivy DS-0026) because it leaves container orchestrators without a reliable application-level mechanism to detect hung or unresponsive containers, increasing the risk of prolonged Denial of Service (DoS) states.
**Learning:** Always include a `HEALTHCHECK` directive in Dockerfiles for services that expose network ports. Use built-in or lightweight standard tools (like Python's `urllib` or a simple script) to verify readiness without unnecessarily expanding the container image's attack surface (e.g., by installing `curl` or `wget` just for healthchecks).
**Prevention:** Incorporate `HEALTHCHECK` as a standard step in the Dockerfile templating and review process. Ensure integration tests (like `test_docker_delivery.py`) actively assert the presence and correct configuration of this directive to prevent regressions.
