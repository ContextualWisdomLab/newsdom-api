...
## 2026-08-26 - [FastAPI Form Size Limit]
**Vulnerability:** Memory Exhaustion / DoS via unbounded `Form` parameters.
**Learning:** `python-multipart` loads the entire form data into memory before route execution or payload size limits are evaluated. Therefore, omitting `max_length` limits to textual `Form` fields causes memory exhaustion if someone maliciously uploads a huge text inside a `Form` field.
**Prevention:** Always add `max_length` limits to textual `Form` fields (e.g., `Form(max_length=50)`) in FastAPI applications to prevent denial-of-service (DoS) via memory exhaustion.
