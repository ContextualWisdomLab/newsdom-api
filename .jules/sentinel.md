...
## 2025-05-24 - Fix Timing Attack in API Token Validation
**Vulnerability:** Length differences in API tokens could be detected before `hmac.compare_digest` is called, potentially enabling a timing attack.
**Learning:** `hmac.compare_digest` leaks length information if the strings are of unequal length. Normalizing lengths by applying hashes is flagged as insecure password hashing by static analysis.
**Prevention:** Execute a dummy constant-time comparison `hmac.compare_digest(credentials, credentials)` when string lengths differ before rejecting the token.
