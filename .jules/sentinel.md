...
## 2026-07-16 - Prevent Timing Attack in Token Length Check
**Vulnerability:** Length checks using short-circuit comparisons (`len(credentials) != len(expected)`) before safe constant-time operations (`hmac.compare_digest`) leak the length of the expected secret via execution time differences, as well as enabling CodeQL heuristics for weak password hashing when naive padding approaches are used.
**Learning:** Comparing sizes before applying a cryptographic digest exposes the secret length. Attempting to balance lengths using `hashlib.sha256()` or `hmac.new()` to normalize sizes triggers security scanners designed to prevent insecure or weak password hashing functions from being used directly.
**Prevention:** If the user-supplied and expected tokens differ in length, execute a dummy constant-time comparison `hmac.compare_digest(expected_token, expected_token)` and then reject the request. This preserves a uniform execution time independent of the client token, without leaking the server token length or relying on disallowed padding routines.
