...
## 2026-08-20 - [FastAPI Form Fields DoS Protection]
**Vulnerability:** Unbounded textual `Form` fields in FastAPI applications using `python-multipart` can lead to denial-of-service (DoS) via memory exhaustion because `python-multipart` loads the entire form data into memory before route execution or payload size limits are evaluated.
**Learning:** Adding `max_length` limits to textual `Form` fields (e.g., `Form(max_length=50)`) prevents large inputs from exhausting server memory.
**Prevention:** Always add `max_length` limits to textual `Form` fields in FastAPI endpoints.
