...
## 2026-09-20 - [MEDIUM] Prevent DoS via Missing Content-Length Bounds in Multipart Parsing
**Vulnerability:** FastAPIs `UploadFile` depends on `python-multipart` which parses and buffers multipart form data into memory before endpoint validation occurs. Without strict upper bounds on the `Content-Length` header in middleware, attackers can send oversized payloads that exhaust server memory before the request reaches the endpoint logic.
**Learning:** Pydantic validation (e.g., `Form(max_length=...)`) occurs *after* the request body has been parsed by Starlette/`python-multipart`. To defend against memory exhaustion and DoS, payload limits must be enforced at the ASGI middleware layer before parsing begins.
**Prevention:** Add middleware that checks the `Content-Length` header against a maximum allowed size (e.g., `MAX_PARSE_UPLOAD_BYTES` + a small buffer for multipart boundaries) and short-circuits the request with a 413 or 400 status code if the limit is exceeded or the header is malformed.
