...
## 2025-02-14 - Prevent Command/Log Injection via Newlines
**Vulnerability:** The blocklist regex for CLI arguments in mineru_runner.py omitted newline (\n) and carriage return (\r) characters, which can allow command or log injection even when shell=False is used.
**Learning:** Shell metacharacter blocklists must explicitly reject newlines, as they can bypass protections in certain toolchain environments.
**Prevention:** Explicitly include \n and \r in the _UNSAFE_CHARS_PATTERN blocklist.
