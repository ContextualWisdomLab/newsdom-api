...
## 2025-03-05 - [MEDIUM] Fix 500 error on non-ASCII bearer tokens
**Vulnerability:** The API crashed with a 500 Internal Server Error when a non-ASCII token was passed in the Authorization header.
**Learning:** `hmac.compare_digest` throws a TypeError when given non-ASCII strings. This can lead to unhandled exceptions and potential DoS.
**Prevention:** Always encode strings to bytes (e.g., UTF-8) before passing them to cryptographic functions like `hmac.compare_digest`.
