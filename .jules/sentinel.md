...
## 2026-08-01 - hmac.compare_digest Unhandled Exception on Non-ASCII (DoS Risk)
**Vulnerability:** The API endpoint `require_authorization` compared `authorization` headers directly as strings using `hmac.compare_digest()`. If a client sends a non-ASCII authorization header, `hmac.compare_digest()` raises a `TypeError`, leading to a 500 Internal Server Error which can be abused for a Denial-of-Service (DoS).
**Learning:** `hmac.compare_digest()` in Python requires strings containing non-ASCII characters to be encoded before comparison.
**Prevention:** Always encode strings to bytes (e.g., `.encode('utf-8')`) before comparing them with `hmac.compare_digest()`.
