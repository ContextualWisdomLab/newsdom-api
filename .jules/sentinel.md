...
## 2026-07-23 - Prevent DoS via Unhandled TypeError in HMAC Comparison
**Vulnerability:** FastAPIs `Header()` can pass non-ASCII characters to `hmac.compare_digest`, which raises a `TypeError` because it does not support comparing strings with non-ASCII characters. This results in an unhandled 500 Internal Server Error, leading to a potential Denial of Service.
**Learning:** When using standard library functions like `hmac.compare_digest` with untrusted client inputs, always ensure the inputs are encoded to bytes to handle arbitrary characters safely and prevent framework-level exceptions.
**Prevention:** Always encode strings to bytes (e.g., `.encode('utf-8')`) before passing them to `hmac.compare_digest()`.
