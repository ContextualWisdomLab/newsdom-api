...
## 2026-08-23 - [High] Path Traversal and DoS in filter_dom
 **Vulnerability:** The script previously accepted arbitrary file paths for both reading input and writing output, creating an unrestricted read/write path traversal risk. Furthermore, there was no file size limit on the parsed JSON input, leaving the application vulnerable to memory exhaustion / DoS attacks when malicious actors supply massive JSON payloads.
 **Learning:** Using `Path` objects is not inherently safe against directory traversal if the tool's execution context acts on user input without bounding checks. Pydantic validation only applies after parsing the entire JSON object into memory, failing to mitigate a pre-validation DoS attack.
 **Prevention:** Ensure input and output paths are resolved and strictly checked using `Path.relative_to()` against trusted directories (like `Path.cwd()` and `tempfile.gettempdir()`). Introduce bounded file size limits (`st_size`) before initiating read and parse operations to prevent OOM errors.
