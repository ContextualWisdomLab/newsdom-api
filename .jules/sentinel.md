...
## 2024-05-18 - Unhandled TypeError DoS via non-ASCII Headers
**Vulnerability:** FastAPIs passing unsanitized request headers to `hmac.compare_digest()` can trigger unhandled `TypeError` exceptions if the client provides non-ASCII characters, resulting in a 500 Internal Server Error (Denial of Service risk).
**Learning:** Python`s `hmac.compare_digest()` does not support comparing strings that contain non-ASCII characters.
**Prevention:** Always encode strings to bytes (e.g., `.encode("utf-8")`) before passing them to `hmac.compare_digest()`, or validate that the inputs are strictly ASCII.
