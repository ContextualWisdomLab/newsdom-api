...
## 2026-08-10 - 🛡️ Fix 500 Internal Server Error in hmac comparison
**Vulnerability:** Application crashed with 500 Internal Server Error when processing an Authorization header with non-ASCII characters due to Python's `hmac.compare_digest()` throwing a `TypeError`.
**Learning:** `hmac.compare_digest()` only supports ASCII strings. Non-ASCII inputs must be encoded to bytes before comparison to prevent unhandled exceptions which lead to server crashes (DoS risk) and leak information about internal states.
**Prevention:** Always encode strings to bytes (e.g., using `.encode('utf-8')`) before comparison with `hmac.compare_digest()`.
