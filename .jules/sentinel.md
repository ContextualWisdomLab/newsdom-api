...
## 2025-05-18 - Prevent 500 Error on Non-ASCII Authorization Header
**Vulnerability:** The application crashed with a 500 Internal Server Error when provided an `Authorization` header containing non-ASCII characters, because `hmac.compare_digest` raises a `TypeError` on non-ASCII strings.
**Learning:** Python's `hmac.compare_digest` only supports comparing ASCII strings or bytes. If string inputs are not strictly validated, it can lead to unhandled exceptions (DoS risk).
**Prevention:** Always encode strings to bytes using `.encode('utf-8')` before passing them to `hmac.compare_digest`.
