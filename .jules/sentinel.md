...
## 2025-03-09 - Prevent Unhandled 500s via Unicode in hmac.compare_digest
**Vulnerability:** The `/parse` API used `hmac.compare_digest(provided, expected)` to check bearer tokens. In Python, `hmac.compare_digest` requires bytes or ASCII-only strings. If an attacker sent a non-ASCII character (like `tøken`) in the `Authorization` header, the endpoint crashed with a `TypeError` (Unhandled 500 error), bypassing normal `401 Unauthorized` handling and potentially allowing resource exhaustion or DoS via error-state manipulation.
**Learning:** Python's `hmac.compare_digest` is strictly bound to ASCII for string inputs. When comparing client-provided string headers (which FastAPI parses and passes as UTF-8/Latin-1 strings), they must always be explicitly encoded to bytes before comparison to ensure no Unicode inputs trigger unhandled type errors.
**Prevention:** Always encode user-provided strings and expected secrets to bytes (e.g. `.encode("utf-8")`) before passing them to cryptographic or low-level C functions like `hmac.compare_digest`.
