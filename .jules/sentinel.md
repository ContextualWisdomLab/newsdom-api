...
## 2025-05-24 - Prevent Length-Leaking Timing Attacks in Token Validation
**Vulnerability:** The API token verification used hmac.compare_digest directly on the provided credentials and the expected token, leaking length through a timing attack.
**Learning:** To prevent timing attacks, a dummy constant-time comparison should be executed if lengths differ.
**Prevention:** Check if the input length matches the expected length, and if not, execute a dummy comparison like hmac.compare_digest(expected, expected) before returning an unauthorized response.
