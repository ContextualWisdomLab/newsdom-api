...
## 2025-07-25 - Prevent DoS via non-ASCII strings in hmac.compare_digest
**Vulnerability:** Passing non-ASCII characters in string inputs to `hmac.compare_digest` raises a `TypeError`, leading to a 500 Internal Server Error when processing untrusted Authorization headers.
**Learning:** `hmac.compare_digest` only supports comparing ASCII strings or byte objects. Untrusted inputs can crash the application or cause DoS via unhandled exceptions if passed directly as strings.
**Prevention:** Always encode strings to bytes (e.g., `.encode('utf-8')`) before using `hmac.compare_digest` to ensure robust constant-time comparison regardless of character encoding.
