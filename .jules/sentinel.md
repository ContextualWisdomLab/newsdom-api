...
## 2024-05-27 - [MEDIUM] Prevent timing attacks when comparing tokens of different lengths
**Vulnerability:** Comparing an arbitrary length credential against a known-length token using `hmac.compare_digest` where the execution time varies depending on whether the strings match in length (early return).
**Learning:** `hmac.compare_digest` protects against timing attacks by taking time proportional to the length of the string, but only if the strings are the same length. If lengths differ, it may return immediately. An attacker could potentially infer the length of the API token by observing response times. Normalizing lengths using SHA256 before comparison triggers aggressive CodeQL heuristics as insecure password hashing.
**Prevention:** Explicitly check if the lengths of the `credentials` and the `encoded_token` are different. If they differ, perform a dummy comparison `hmac.compare_digest(credentials, credentials)` to safely balance the execution time, and then return an unauthorized response. Only perform the real `hmac.compare_digest` if the lengths are exactly the same.
