...
## 2026-08-25 - Prevent DoS via FastAPI Form field memory exhaustion
**Vulnerability:** FastAPI `Form` fields lack length constraints, allowing DoS via memory exhaustion as `python-multipart` loads the entire form data into memory.
**Learning:** Always explicitly set `max_length` for string `Form` parameters.
**Prevention:** Enforce `max_length` in all textual `Form` field definitions.
