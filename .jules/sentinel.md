...
## 2026-08-30 - Form Parameter Validation & Multipart Allocation Clarification
**Vulnerability:** FastAPI `Form` fields for `language` and `mode` lacked `max_length` constraints, which could theoretically allow large inputs. However, adding `max_length` to fields *after* parsing does NOT prevent memory exhaustion DoS, because `python-multipart` consumes memory for the entire request body before route validation runs.
**Learning:** Pydantic `Form` validation (`max_length`) acts as an application logic boundary, not a primary pre-parser memory defense. True multipart DoS protection must be implemented via admission control (e.g., maximum request body limits) *before* the multipart parser handles the payload. The `max_length=50` constraint provides a narrow application-level invariant.
**Prevention:** Always combine application-layer bounds like `Form(max_length=...)` with pre-parser request size limits (like the existing 21 MiB boundary) to achieve defense-in-depth, and accurately document the layer each defense operates at.
