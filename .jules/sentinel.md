...
## 2024-08-05 - Fix DoS via TypeError in hmac.compare_digest
**Vulnerability:** Passing non-ASCII characters in the Authorization header to FastAPI triggers a 500 Internal Server Error, because Python's `hmac.compare_digest` raises a `TypeError` when comparing non-ASCII strings.
**Learning:** `hmac.compare_digest` is strictly for bytes or ASCII-only strings. If untrusted input could contain arbitrary unicode, it must be encoded to bytes before comparison to avoid unhandled framework-level exceptions.
**Prevention:** Always encode strings to bytes (e.g., using `.encode('utf-8')`) before using `hmac.compare_digest()`.
