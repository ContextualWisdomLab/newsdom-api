...
## 2026-08-11 - [MEDIUM] Apply security headers to FastAPI HTTPExceptions

**Vulnerability:** Unhandled `HTTPException` responses (e.g. 4xx, 5xx generated by FastAPI) bypass standard middleware and custom generic exception handlers, failing to include defense-in-depth security headers like CSP and X-Frame-Options.
**Learning:** In FastAPI, the default `http_exception_handler` intercepts `HTTPException` directly. You must explicitly override it by registering a custom handler to ensure headers are applied to all error paths.
**Prevention:** Use `application.add_exception_handler(HTTPException, custom_handler)` to ensure all HTTP error responses are routed through a shared security-header helper.

## 2026-08-11 - [MEDIUM] FastAPI HTTPException에 보안 헤더 적용 및 UnicodeEncodeError 처리
**Vulnerability:**
1. FastAPI에서 발생하는 기본 `HTTPException` 응답(4xx, 5xx)이 미들웨어나 일반 예외 처리기를 우회하여, CSP나 X-Frame-Options와 같은 방어적 보안 헤더가 누락되는 취약점이 있습니다.
2. `/parse` 엔드포인트의 Authorization 헤더 유효성 검사 중 토큰을 UTF-8로 인코딩할 때 올바르지 않은 UTF-8 바이트 시퀀스가 포함되어 있으면 `UnicodeEncodeError`가 발생하여 서비스 장애나 정보 유출이 발생할 수 있습니다.
**Learning:**
1. FastAPI에서는 예외 발생 시 전역 미들웨어를 거치지 않는 경우가 있으므로 `HTTPException`에 대한 사용자 정의 처리기를 별도로 등록해야 합니다.
2. `token.encode("utf-8")`와 같이 문자열 인코딩 시 예외 처리를 누락하면 악의적인 입력에 의해 500 에러를 유발할 수 있습니다.
**Prevention:**
