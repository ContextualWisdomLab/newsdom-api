...
## 2025-05-24 - [MEDIUM] Fix Token Length Timing Attack Vulnerability
**Vulnerability:** In `src/newsdom_api/main.py`, the token verification logic immediately returned `False` if the length of the provided `credentials` did not match the expected `token` length. This early return created a timing discrepancy, potentially allowing an attacker to deduce the expected token's length by measuring response times.
**Learning:** Even when using `hmac.compare_digest` for the actual string comparison, validating lengths upfront with a fast path (e.g., `if len(a) != len(b): return False`) re-introduces a timing leak for the length attribute itself.
**Prevention:** If lengths differ, execute a dummy constant-time comparison (e.g., `hmac.compare_digest(expected_token, expected_token)`) before returning `False`. This ensures the execution time is balanced regardless of whether the lengths matched, without tying the comparison time to an attacker-controlled input length.
