...
## 2026-07-31 - Prevent 500 Errors (DoS) via Non-ASCII Header Validation
**Vulnerability:** Comparing an external HTTP header against a secret using `hmac.compare_digest` on raw strings raises a `TypeError` and 500 error if non-ASCII characters are present, leading to potential DoS vulnerabilities.
**Learning:** Python's `hmac.compare_digest` only supports ASCII characters when given strings.
**Prevention:** Always encode user-supplied strings and the expected secret to bytes (e.g., `.encode('utf-8')`) before using `hmac.compare_digest`.
