...
## 2026-10-24 - Prevent Length Extension / Timing Attacks in Authentication
**Vulnerability:** Length-hiding string comparisons using `hmac.compare_digest` can still leak the expected string length if the comparison returns early when lengths differ. When lengths were mismatched, the previous implementation directly returned unauthorized, leaking that the guessed length was incorrect via timing.
**Learning:** `hmac.compare_digest` only provides constant-time comparison for strings of the *same length*. If lengths differ, it may return immediately or take a different amount of time, allowing an attacker to brute-force the length of the secret token.
**Prevention:** Normalize execution time by always performing a dummy constant-time comparison (e.g. `hmac.compare_digest(credentials, credentials)`) when lengths differ, ensuring the response time does not leak length information.
