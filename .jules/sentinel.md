...
## 2026-08-09 - Prevent 500 error via HMAC non-ASCII comparison
**Vulnerability:** Comparing strings with non-ASCII characters directly in `hmac.compare_digest()` raises a `TypeError` which can lead to an unhandled 500 Internal Server Error, causing a potential denial of service (DoS) or leaking server internals.
**Learning:** Python's `hmac.compare_digest` requires both strings to be converted into bytes (e.g., using `.encode('utf-8')`) when validating untrusted inputs, because non-ASCII characters cannot be directly compared and will cause the application to crash.
**Prevention:** Always ensure that strings passed to `hmac.compare_digest()` are converted to bytes first, especially when handling untrusted user input such as the `Authorization` header.
