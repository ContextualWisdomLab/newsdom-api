...
**Vulnerability:** Framework-generated 404 and 405 responses used Starlette's default HTTP exception handler rather than the service-owned error boundary. The outer middleware already added the standard headers, but relying on that incidental stack ordering left error serialization and header authority split across two handlers.
**Learning:** FastAPI's `HTTPException` is a subclass used by application code, while routing errors are raised as `starlette.exceptions.HTTPException`. Registering only the subclass does not replace Starlette's default 404 and 405 handler.
**Prevention:** Register the secured handler for `StarletteHTTPException`, retain FastAPI's subclass for application-raised errors, and test both the handler map and real 404 and 405 responses.
