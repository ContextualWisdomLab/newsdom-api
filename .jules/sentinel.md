...
## 2025-02-24 - [Disk Exhaustion via Interrupted Uploads]
**Vulnerability:** The temporary file created for parsing PDF uploads was not reliably cleaned up if a client disconnected or a network error occurred during the asynchronous chunked read (`await file.read(8192)`). This could lead to a disk exhaustion DoS vulnerability.
**Learning:** The file cleanup `tmp_path.unlink()` was previously placed in a `finally` block that was only entered *after* the reading loop had successfully completed. Because network interruptions during the reading loop raise exceptions before the `try` block protecting the parsing logic was even entered, the temporary file was orphaned on the disk.
**Prevention:** Always wrap the entire lifecycle of a temporary file—including its creation context and any asynchronous network operations that populate it—within a comprehensive `try...finally` block that ensures deletion, testing for its existence if necessary.
