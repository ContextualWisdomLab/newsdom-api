...
## 2026-08-02 - Fix 500 Internal Server Error vulnerability on non-ASCII Authentication header
**Vulnerability:** Passing strings containing non-ASCII characters directly to `hmac.compare_digest()` raises a `TypeError` and leads to a 500 Internal Server Error, creating a Denial of Service vulnerability.
**Learning:** In Python, `hmac.compare_digest()` only accepts ASCII strings. To prevent unhandled exceptions and 500 Internal Server Error (DoS) vulnerabilities, always encode strings to bytes before comparing them.
**Prevention:** Always encode string variables (e.g. `header.encode('utf-8')`) before using them in `hmac.compare_digest()`.
