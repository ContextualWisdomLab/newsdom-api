...
async def limit_upload_size(request: Request, call_next):
    ...
    async def bounded_receive():
        ...
        if message["type"] == "http.request":
            ...
            if bytes_received > MAX_BYTES:
                raise RuntimeError("PayloadTooLargeException")
        return message
    ...
    try:
        ...
        if str(e) == "PayloadTooLargeException":
            return JSONResponse(status_code=413, content={"detail": "Payload Too Large"})
        raise
...
resp = httpx.post("http://127.0.0.1:8004/", content=generate_chunked(), headers={"Content-Type": "multipart/form-data; boundary=boundary"})
