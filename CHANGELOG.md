...
### Changed
- `/parse`의 `language`와 `mode` multipart 필드는 OpenAPI 3.1 Schema Object의 plural `examples` 배열로 실제 기본값을 안내하며, deprecated singular `example` metadata를 더 이상 발행하지 않습니다.
