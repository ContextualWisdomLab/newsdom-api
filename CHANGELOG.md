...
### Changed
- `/parse` now accepts PDFs up to 64 MiB, matching naruon's signed email-import
  transport ceiling while retaining bounded streaming reads, temporary-file
  cleanup, and the existing `413 Payload Too Large` response above the limit.
