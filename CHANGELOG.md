...
### Added
- [CLI] NewsDOM JSON 파일을 특정 키워드로 필터링하는 `tools/filter_dom.py` 도구를 추가했습니다.
- [CLI] 두 NewsDOM JSON 파일의 구조적 차이를 비교하는 `tools/compare_dom.py` 도구를 추가했습니다.
- [CLI] NewsDOM JSON 파일의 공백을 제거하여 용량을 최소화하는 `tools/minify_dom.py` 도구를 추가했습니다.
