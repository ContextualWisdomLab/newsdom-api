...
### Added
- [CLI] 생성된 NewsDOM JSON 결과물이 `ParseResponse` Pydantic 스키마를 만족하는지 검증하는 `tools/validate_dom.py` 도구를 추가했습니다.
- [CLI] NewsDOM JSON에서 순수 텍스트만 추출하여 텍스트 파일로 저장하는 `tools/extract_text.py` 도구를 추가했습니다.
