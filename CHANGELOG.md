...
### Added
- 열린 PR과 활성·미확정 Agent Task가 모두 없을 때만 하나의 구매자 체감 제품 개선 작업을 생성하는 fail-closed 시간별 제품개발 워크플로를 추가했습니다. 작업은 테스트 우선, production statement/branch coverage 100%, 전체 docstring, 보안·패키징·문서 검증, CHANGELOG 갱신 및 단일 `develop` PR을 요구하며 병합·배포·릴리스 우회를 금지합니다.
