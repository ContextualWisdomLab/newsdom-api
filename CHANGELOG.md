...
### Added
- [CLI] DOM JSON 데이터에서 기사 제목(headline)만 추출하여 텍스트 파일로 저장하는 `tools/extract_headlines.py` 도구를 추가했습니다.
- [CLI] DOM JSON 데이터에서 띄어쓰기를 최소화하여 JSON 크기를 줄이는(Minify) `tools/minify_dom.py` 도구를 추가했습니다.
