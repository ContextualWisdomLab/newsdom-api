...
### Changed
- `parse` 엔드포인트의 폼 데이터 파라미터(`file`, `language`, `mode`)에 명시적인 `json_schema_extra={"example": ...}`를 추가하여 Swagger UI와 OpenAPI 명세 상의 개발자 경험(DX)을 개선했습니다. Pydantic V2와의 호환성 문제 없이 예제값이 올바르게 렌더링됩니다.
