...
### Added
- [CLI] `tools/export_markdown.py`와 `tools/export_html.py` 도구에 스키마의 누락된 메타데이터(문서 파싱 상태 `quality`, 페이지 크기 `width/height`, Bounding Box `bbox`, 이미지 미디어 타입 `media_type`) 추출 및 포맷팅 기능을 추가했습니다.
