...
### Changed
- [UX/DX] OpenAPI Swagger UI에서 모델 속성 명확성을 개선하기 위해 `ArticleNode` 및 `PageNode` 필드(`body_blocks`, `width`, `height`, `ads`, `headers` 등)에 `json_schema_extra` 예시 데이터를 추가하고, `headline` 필드에서 불필요한 `...` 마커를 제거했습니다. (Palette)
...
### Security
- Trivy 스캐너에서 발견된 `pypdf` 의존성 보안 취약점(CVE-2026-71852, CVE-2026-71870)을 수정하기 위해 버전을 `6.15.0`으로 올렸습니다. (Sentinel)
