...
### Added
- 로컬 테스트 환경 개선을 위해 `development` 런타임 프로필에서 실행 시 Swagger UI의 `persistAuthorization` 옵션을 활성화했습니다.
