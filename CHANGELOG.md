...
### Security
- `/parse` Bearer 검증에서 제공 credential 길이가 expected token과 다를 때 expected-length dummy `compare_digest`를 실행해 token 길이 기반 timing 정보 노출 가능성을 낮추고 기존 401 fail-closed semantics를 유지합니다.
