...
### Changed
- OpenAPI Swagger UI 설정(`swagger_ui_parameters`)에 `"persistAuthorization": True`를 추가하여, 개발자 환경(`runtime_profile="development"`)에서 페이지를 새로고침해도 인증 토큰(Bearer Token 등)이 유지되도록 개발자 경험(DX)을 개선했습니다.
