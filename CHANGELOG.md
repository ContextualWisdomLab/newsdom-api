...
### Added
- [CLI] NewsDOM JSON 파일 내에서 특정 문자열을 찾아 일괄 치환하는 `tools/find_replace_dom.py` 도구를 추가했습니다.
...
### Security
- CI Trivy 파일시스템 스캐너가 보고한 `pillow` 패키지의 보안 취약점을 해결하기 위해 `uv.lock`에서 버전을 업데이트했습니다.
