...
**Learning:** Adding concrete examples to Pydantic V2 `Field` definitions significantly improves Developer Experience (DX) for backend-only APIs by automatically populating realistic, copy-pasteable data in Swagger UI. However, using the `examples` property directly in JSON schema can cause compatibility issues with OpenAPI 3.0.x and specific Swagger tooling that expects a singular `example` key at the property level.
**Action:** When working on FastAPI schema definitions, proactively inject `json_schema_extra={"example": "value"}` into `Field(...)` declarations for all fields to guarantee the generated OpenAPI documentation is intuitive, self-documenting, and widely compatible across all Swagger tools, while avoiding Pydantic V2 deprecation warnings for the bare `example=` argument.
