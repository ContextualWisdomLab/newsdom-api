...
def test_parse_rejects_non_ascii_bearer_when_secret_set(monkeypatch, stub_parser):
    monkeypatch.setenv(API_TOKEN_ENV_VAR, "s3cret-token")
    client = TestClient(app)
    # httpx/TestClient requires headers to be ascii string or bytes.
    # We pass it as bytes to simulate a client sending non-ASCII bytes.
    response = client.post(
        "/parse",
        files=_PDF_FILES,
        headers={"Authorization": "Bearer ë".encode("utf-8")},
    )
    assert response.status_code == 401
    assert response.json()["detail"] == "Unauthorized"
