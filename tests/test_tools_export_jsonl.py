...
def test_export_jsonl_preserves_published_output_when_encoding_fails(
    ...
    # Pre-populate output file to check if it's preserved on failure
    ...
    # Mocking os.replace to raise an exception
    import os
    ...
    os.replace = mocked_replace
    try:
        with pytest.raises(OSError, match="Mocked Error"):
            export_jsonl(input_file, output_file)

        # Verify the original file was preserved
