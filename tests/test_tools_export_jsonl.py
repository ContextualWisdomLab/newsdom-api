...
def test_export_jsonl_success(tmp_path: Path) -> None:
    ...
    with output_file.open("r", encoding="utf-8") as f:
        ...
        assert record0["document_id"] == "test_doc"
        assert record0["page_number"] == 1
        assert record0["article_id"] == "art_1"
        assert record0["headline"] == "Test Headline 1"
        assert record0["body_blocks"] == ["Block 1", "Block 2"]

        record1 = json.loads(lines[1])
        assert record1["article_id"] == "art_2"
        assert record1["body_blocks"] == []

        record2 = json.loads(lines[2])
        assert record2["page_number"] == 2
        assert record2["article_id"] == "art_3"
        assert record2["body_blocks"] == ["Block 3"]
