...
VALID_JSON_DATA = {
    ...
    "pages": [{"page_number": 1, "articles": [{"headline": "Title"}]}],
}
...
def test_export_jsonl_success(tmp_path: Path) -> None:
    ...
    assert json.loads(content) == VALID_JSON_DATA
...
def test_export_jsonl_preserves_published_output_when_encoding_fails(
    ...
    with pytest.raises(ValueError, match="Simulated encoding error"):
        export_jsonl(input_file, output_file)

    assert output_file.read_text() == "existing data"
