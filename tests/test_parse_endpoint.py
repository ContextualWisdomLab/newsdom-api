...
def test_parse_endpoint_rejects_large_content_length_header_value_error(monkeypatch):
    monkeypatch.setattr("newsdom_api.main._validate_pdf_structure", lambda *a: None)

    # We must mock parse_pdf since if the endpoint is not rejected by content length, it proceeds to parse
    def fake_parse_pdf_bytes(file_path, filename, **kwargs):
        return {"document_id": "fixture", "pages": []}
    monkeypatch.setattr("newsdom_api.main.parse_pdf", fake_parse_pdf_bytes)
...
def test_parse_endpoint_accepts_valid_content_length_header(monkeypatch):
    monkeypatch.setattr("newsdom_api.main._validate_pdf_structure", lambda *a: None)
...
def test_parse_endpoint_accepts_no_content_length_header(monkeypatch):
    monkeypatch.setattr("newsdom_api.main._validate_pdf_structure", lambda *a: None)
    ...
    # We need to simulate a request with no content-length header
    from fastapi.testclient import TestClient
    ...
    # Just run the middleware directly with a mock request
    from newsdom_api.main import security_boundary_middleware

    # ... actually it's easier to just use TestClient but drop the header in a middleware, or just use the app and send a chunked request so content-length is missing.

    client = TestClient(app)

    def chunked_generator():
        yield b"--boundary\r\n"
        yield b'Content-Disposition: form-data; name="file"; filename="fixture.pdf"\r\n'
