...
def test_parse_pdf_bytes_sanitizes_special_characters(monkeypatch):
    observed = {}
    ...
    monkeypatch.setattr("newsdom_api.service.run_mineru", fake_run_mineru)
    result = parse_pdf_bytes(b"pdf-bytes", filename="file ; echo 'hacked'.pdf")
    assert observed["path_name"] == "file___echo__hacked_.pdf"
    assert result.document_id == "file___echo__hacked_"


def test_parse_pdf_bytes_sanitizes_only_special_characters_returns_default(monkeypatch):
    observed = {}
