...
`PageNode.page_number`, the optional page-geometry and repeated text-block fields
in `PageNode`, plus `ImageNode.media_type`, publish examples through Pydantic's
plural `Field(examples=[...])` metadata.
...
- that the canonical page number is one-based;
