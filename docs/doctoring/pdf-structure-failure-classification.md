...
If the narrow exception tuple causes a supported malformed PDF to escape as a server error, add only the evidence-backed parser exception class after reproducing it with a bounded fixture. Do not restore `except Exception`, and do not weaken upload, admission, timeout, or process resource controls.
