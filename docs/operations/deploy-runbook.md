...
- Isolated validation uses a 5-second wall-clock limit, 5 CPU seconds, a
  512 MiB address-space cap, disabled core dumps, and a 64-process cap in
  a disposable Linux child. A signal-killed child is a 415, not a 503.
  On 415, re-export the document as a standard PDF and retry. On validator
  503, confirm the replica is Linux with working `setrlimit`, then roll
  back to the previous image if the child cannot spawn.
