...
exact head의 JSONL contract tests와 전체 repository test/lint/type/security checks가 terminal GREEN이어야 한다. 단순 line count, synthetic fixture coverage percentage, predecessor head의 check 성공은 canonical fidelity 증거를 대신하지 않는다. `934da87ae774a46a80ff681555015370f63388d3`의 중앙 Actions 결과는 아직 terminal GREEN이 아니므로 merge-ready로 간주하지 않는다.
