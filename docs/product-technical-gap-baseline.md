...
이 조치는 **fixed temporary-path symlink overwrite와 부분 output 관찰 경계**를 좁혀 고친 것입니다. output directory 자체가 공격자에 의해 교체되는 모든 pathname race, directory metadata durability, 여러 writer가 같은 최종 output을 동시에 publish할 때의 business-level arbitration까지 해결했다고 주장하지 않습니다.
...
1. `ParseResponse` validation failure는 output을 부분적으로 쓰지 않고 non-zero exit로 끝납니다.
...
- exact-head CI에서 page selector와 secure temp-write regression을 검증합니다.
